import{j as e}from"./jsx-runtime.b9e88e07.js";import{r as n}from"./index.03be2d59.js";/* empty css                       */import{m as O}from"./motion.9b287751.js";const G=({program:d,listType:m,index:a})=>{const[g,r]=n.useState(!1),[w,i]=n.useState(!0),j=m=="list";n.useEffect(()=>{setTimeout(()=>i(!1),1200)});const{degreeType:c,slug:N,school:p,programOptions:k,additionalOptions:x,programAtAGlance:$,learnMoreList:y}=d.program,{title:C}=d;return e.jsx("div",{className:`program-card filter  hover:drop-shadow-md drop-shadow-none transition-all duration-300 bg-white ${g?"active-hover":""}
      ${w?"pulse":""}  
      `,id:N,onMouseEnter:()=>r(!0),onMouseLeave:()=>r(!1),children:e.jsxs(O.a,{className:`program-link block border-[1px] 
...
scale-[.975]
        ...
        `,id:`${a?"open-list":""}`,children:d.map((r,w)=>e.jsx("div",{className:"drop-down-item",children:r},w))})]})}const A=({filteredData:d,boxId:m,checkboxContent:a})=>e.jsx(e.Fragment,{children:a&&a.map((g,r)=>e.jsx("div",{className:`drop-down-item ${m}`,children:g},r))});function Q({data:d,areaOfStudy:m,programOptionsClean:a,additionalOptions:g}){const[r,w]=n.useState(d.programs.nodes),[i,j]=n.useState([]),[c,N]=n.useState([]),[p,k]=n.useState([]),[x,$]=n.useState(""),[y,C]=n.useState(null),[l,f]=n.useState(""),h=n.useRef(null),u=t=>{C(y===t?null:t)},S=t=>{if(t.type==="click"&&h.current&&!h.current.contains(t.target)||t.type==="keydown"&&t.key==="Escape"){C(null);const s=document.getElementById("open-list");s&&s.removeAttribute("id")}};n.useEffect(()=>(document.addEventListener("click",S),document.addEventListener("keydown",S),()=>{document.removeEventListener("click",S),document.removeEventListener("keydown",S)}),[]),n.useEffect(()=>{B()},[i,c,p,x]);const B=()=>{let t=d.programs.nodes.filter(I);i.length>0&&(t=t.filter(z)),c.length>0&&(t=t.filter(V)),p.length>0&&(t=t.filter(H)),t=t.sort((s,v)=>s.title.localeCompare(v.title)),w(t)},I=t=>{const s=x.toLowerCase();return t.title.toLowerCase().includes(s)||t.program.school.toString().toLowerCase().includes(s)},z=t=>(i??[]).includes(t.program.school.toString()),V=t=>c.some(s=>(t.program.programOptions??[]).includes(s)),H=t=>p.some(s=>(t.program.additionalOptions??[]).includes(s)),E=t=>L(t.target.value,j,i),D=t=>L(t.target.value,N,c),P=t=>L(t.target.value,k,p),L=(t,s,v)=>{let o=[...v];o.includes(t)?o=o.filter(T=>T!==t):o.push(t),s(o)},M=t=>{$(t.target.value)},R=()=>{$("");const t=document.getElementById("text-filter");t&&(t.value="")},b=(t,s,v)=>t.map((o,T)=>e.jsxs("p",{className:`${s.name}-filter input-container 
        ...
        lg:px-0
        md:pb-[100px]
        `,children:r&&r.map((t,s)=>e.jsx(G,{program:t,listType:l,index:s},t.program.slug+s))})]})}export{Q as default};
